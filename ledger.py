...
def plotInventory(inventory: Inventory) -> None:
  def isCompletelySold(row) -> bool:
    if row["Quantity"] == 0:
      return True
    return False

  inventory["Completely Sold"] = inventory.apply(lambda row: isCompletelySold(row), axis=1)
  rel = sns.scatterplot(data=inventory, x="Purchase Price", y="Quantity",
           hue="Completely Sold", 
           style="Completely Sold")
  rel.set(title="ADA Price by Quantity")
  plt.show()
...
if __name__ == '__main__':
  ...
  report, inventory = f(ledger=ledger, quantity=2000, price=1.45)
  currentQuantity = inventory["Quantity"].sum()
  for r in report:
    print(str(r))
